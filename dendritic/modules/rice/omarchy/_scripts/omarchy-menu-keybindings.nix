  ...
  text = ''
    ...
    hl = setmetatable({
      ...
      bind = function(keys, bind_dispatcher, opts)
        ...
        print(table.concat({ tostring(modmask), description, key, kind, arg }, "\31"))
      ...
      ' |
        while IFS=$'\x1f' read -r modmask key keycode description dispatcher arg; do
          ...
          if [[ -z $description && -n $key ]]; then
            description="''${LUA_BIND_DESC_MAP["$modmask,$key"]:-}"
          fi

          if [[ $dispatcher == "__lua" && -n $key ]]; then
            if [[ -n $description ]]; then
              cache_key="$modmask,$description,$key"
            else
              cache_key="$modmask,$key"
            fi
            dispatcher="''${LUA_BIND_DISPATCHER_MAP["$cache_key"]:-$dispatcher}"
            arg="''${LUA_BIND_ARG_MAP["$cache_key"]:-$arg}"
          fi
    ...
    # Actions Omarchy binds to a second chord meant as an alternative, named one at a time.
    ...
    # Drop repeated records while keeping the order Hyprland reported them in
